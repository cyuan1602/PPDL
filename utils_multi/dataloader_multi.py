...
def LoadDataset_Keypoint(args):
    ...
    des_clean = des_all[val_idx]
    # des_clean = des_clean[des_clean['loc_y'] > 11.5 ]
    # train-test split
    ...
    # 先过滤掉 loc_y >= 8.5 的样本
        # des_clean = [d for d in des_clean if d['loc_y'] < 8.5]
        random.Random(22).shuffle(des_clean)
        split_idx = round(len(des_clean) * 0.8)
        des_train = des_clean[:split_idx]
        des_test = des_clean[split_idx:]
        # des_test=[d for d in des_test if d['loc_y']>11.5]
    # 断言：过滤后的总数 = 训练+测试
    ...
    ## subject independent split
        ...
        des_test = des_clean[val_test]
        des_test = des_test[des_test['loc_y'] < 8.5]   # 你要的这一句（注意这里 des_test 是 DataFrame）
        des_test = des_test.to_dict('records')
        # assert len(des_clean)==(len(des_train)+len(des_test))
